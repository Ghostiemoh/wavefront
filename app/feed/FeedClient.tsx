...
interface FeedClientProps {
  ...
  initialNewListings: NewListingToken[];
  initialNarrativeError: boolean;
}
...
export function FeedClient({
  ...
  initialTrending,
  initialNewListings,
  initialNarrativeError,
}: FeedClientProps) {
  const [narratives, setNarratives] = useState<NarrativeEvent[]>(initialNarratives);
  const [trending, setTrending] = useState<TrendingToken[]>(initialTrending);
  const [newListings, setNewListings] =
    useState<NewListingToken[]>(initialNewListings);
  const [lastRefresh, setLastRefresh] = useState<string | null>(null);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [narrativeError, setNarrativeError] = useState(initialNarrativeError);
        ...
        {trending.length === 0 ? (
          <div className="p-6 rounded-xl bg-bg-surface border border-border text-center">
            <p className="text-text-muted text-sm">
              Birdeye trending feed is quiet right now. Auto-retrying every 30s.
            </p>
          </div>
        ) : (
